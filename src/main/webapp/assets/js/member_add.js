$(function(){
    ...
    $("#add").click(function(){
        ...
        let user_pwd_confirm = $("#mi_pwd_con").val();
        if(user_pwd != user_pwd_confirm){
            alert("비밀번호와 비밀번호 확인이 일치하지 않습니다.");
            return;
        }
        let user_name = $("#mi_name").val();
        ...
        const patternEmail = /^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i; // 이메일 검사하는 정규표현식
        let user_email = $("#mi_email").val();
        ...
        let user_addr = $("#mi_addr").val();
        let user_birth_year = $("#mi_birth_year").val();
        let user_birth_month = $("#mi_birth_month").val();
        let user_birth_date = $("#mi_birth_date").val();
        let user_phone = $("#mi_phone").val();
        let user_pay_card = $("#mi_pay_card").val();
        let user_pay_acc = $("#mi_pay_acc").val();

        if(!inputValidation(user_addr, "주소")) {return;}
        ...
        // if(inputValidation(user_pay_card, "카드번호")) {return;}
        // if(inputValidation(user_pay_acc, "계좌번호")) {return;}
        let user_gen = $("#mi_gen option:selected").val();
        // 20210614
        // 2021614
        // 202161
        // 2021  006
        ...
        let data ={
            ...
            mi_name:user_name,
            mi_email:user_email,
            mi_address:user_addr,
            mi_birth:birth,
            mi_gen:user_gen,
            mi_phone:user_phone,
            mi_pay_card:user_pay_card,
            mi_pay_account:user_pay_acc
        };
    ...
    $("#doub_ck2").click(function(){
        ...
        $.ajax({
            type:"get",
            url:"/member/checkEmail?email="+user_email,
            success:function(r){
                // 요청결과로 상태값이 200이나 202가 나왔을 때 실행
                alert(r.message);
                emailCheck = r.status;
